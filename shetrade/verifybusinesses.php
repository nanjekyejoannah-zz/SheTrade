...
<title>Verify Businesses | All about Women owned Businesses and More</title>
  ...
  <tr>
    <td><table width="723" border="0" align="right" bgcolor="#FFFFFF">
      <tr>
        ...
        <h1>Verify Businesses</h1>
        <?php
		///Delete Action
		if(isset($_GET['verify'])){
			echo "<p><center>";
			mysql_query("UPDATE animal SET Verified='Yes' WHERE Animals_ID='$_GET[verify]'", $conn);
			echo "<alart>Business Successfully Varified</alart><br>";
			...
			if(isset($_GET['member'])){
				...
				$msg = "Hallo $Title $Last_Name\n";
				$msg .= "Your animal has been Verified\n";
				$msg .= "Its available to all site visitors at the link; \n";
				$msg .= "http://www.animalminute.com/details.php?animal=$_GET[verify]\n\n";
				
				//$msg .= "You can edit it at(after you login);\n";
				//$msg .= "www.ugautos.com/car/edit.php?id=$_GET[id]\n";
				
				$msg .= "View a list of your Businesses that have not yet been verified:\n";
				$msg .= "http://www.mediaminute.net/shetrade/notverified.php\n\n";
				
				
				$msg .= "_______________________________\n";
				$msg .= "She Trade Uganda\n";
				$msg .= "http://www.mediaminute.net/shetrade/\n";
				$msg .= "info@mediaminute.net\n";
			...
			echo "<alart>Car Not Varified, Dealer is on the Blacklist</alart><br>";	
			}
			*/
			echo "</center></p>";
		}
		...
		$totalcars = "SELECT Animals_ID FROM animal WHERE Verified='No'";
		$totalresults = mysql_query($totalcars);
		$total=mysql_num_rows($totalresults);
		$total_pgs = ceil($total / $max_results);
		
		$sql = "SELECT Animals_ID, Animal, Breed, Image_1, Price, Added_By FROM animal WHERE Verified='No' LIMIT $start_from, $max_results";
		...
		if($max_results>$total){
			echo "<br>Showing $itemstart - $total of $total Animals<br><br>";
			}
		else{
			echo "<br>Showing $itemstart - $this_page_total of $total Animals<br><br>";
			}
		}else{
		...
		while ($newArray = mysql_fetch_array($result)) {
			...
			if ($Animals_ID!=""){
				...
				echo "<table width=\"100%\" border=\"0\">";
				  ...
				  echo "<tr>";
					echo "<td>&nbsp;</td>";
					echo "<td><a href=\"verifybusinesses.php?page=$page&verify=$Animals_ID&member=$Added_By\">Verify</a></td>";
				  echo "</tr>";
				  echo "<tr>";
					echo "<td>Added by</td>";
					echo "<td><a href=\"edit.php?animal=$Animals_ID\">Edit Animal</a></td>";
				  echo "</tr>";
				  echo "<tr>";
					echo "<td><a href=\"details.php?member=$Added_By\"><strong>$Title $First_Name $Last_Name</strong></a></td>";
					echo "<td><a href=\"delete.php?animal=$Animals_ID\"><alart>Delete Animal</alart></a></td>";
				  echo "</tr>";
				echo "</table>";
				echo "<br /><hr />";
				echo "</p>";
			...
			if ($page==1){
				...
				$prev=$page-1;
				echo "<a href='verifybusinesses.php?page=$prev'>Prev</a>";
			}
			echo " | ";
			if ($page==$total_pgs){
				echo "Next";
				...
				$next=$page+1;
				echo "<a href='verifybusinesses.php?page=$next'>Next</a>";	
			}
			echo " | ";
			//echo "Prev | Next | ";
			for ($i=1; $i<=$total_pgs; $i++) { 
				if($page==$i){
					...
					echo "<a href='verifybusinesses.php?page=".$i."'>".$i."</a> "; 
				}
			}
			}
      ...
      <tr>
        ...
        <a href="addbusiness.php"><strong>Add Animal</strong></a><br />
        <a href="addproduct.php"><strong>Add Product</strong></a><br />
        <a href="mylisting.php?view=businesses"><strong>Animal List</strong></a><br />
        <a href="mylisting.php?view=products"><strong>Product List</strong></a><br />
        <a href="notverified.php?view=businesses"><strong>Not Yet Verified Animals</strong></a><br />
