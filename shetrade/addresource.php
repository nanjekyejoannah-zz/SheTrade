  ...
  <tr>
    <td><table width="723" border="0" align="right" bgcolor="#FFFFFF">
      <tr>
        ...
        <h2>Add Resource</h2>
        <?php 
		if(isset($_GET['add']) && $_GET['add']=="res"){
			echo "<center>";
			if($_POST['resource']!=""){
				$Add_Country=mysql_query("INSERT INTO `animal_minute`.`resource` (`Resource_ID`, `Category`, `Resource`, `Description`, `Link`) VALUES (NULL, '$_POST[category]', '$_POST[resource]', '$_POST[description]', '$_POST[link]');", $conn);
				echo "<alart>Resource <em><strong>'$_POST[resource]'</strong></em> was Added</alart>";
			}else{
				echo "<alart>Please enter a Resource</alart>";
			}
			echo "</center>";
		}
		?>
        <p>Add a Resource, its Category, its Description, External weblink(if any) and Click 'Add Recourse'</p>
        <div id="contact_form">
        <form action="addresource.php?add=res" method="post" enctype="multipart/form-data">
        	<label for="resource"><strong>Resource*:</strong></label>
            <input type="text" name="resource" class="required input_field" value=""/>
            <div class="cleaner h10"></div>
            
            <label for="category"><strong>Category:</strong></label>
            <select name="category" class="main_input">
            <?php
				$Category_Result=mysql_query("SELECT Category FROM resource_category ORDER BY Category ASC", $conn);
				//$Member_Details= mysql_fetch_array($Member_Result);
            	while ($Category_Array = mysql_fetch_array($Category_Result)) {
					$Category_DB=$Category_Array['Category'];
					echo "<option value=\"$Category_DB\">$Category_DB</option>";
				}
			?>
            </select>
            <div class="cleaner h10"></div>
            ...
            <textarea name="description" cols="" rows="3"></textarea>
            <div class="cleaner h10"></div>
            
            <label for="link"><strong>External Link:</strong></label>
            <input type="text" name="link" class="required input_field" value=""/>
            <div class="cleaner h10"></div>
            
        	<input type="submit" value="Add Recourse" id="submit" name="submit" class="submit_btn float_l" /> 
        </form>
        </div>
        <div class="cleaner"></div>     
        <br /><br /><br /><hr />
        <h2>Add Resource Category</h2>
        <?php 
		if(isset($_GET['add']) && $_GET['add']=="cat"){
			echo "<center>";
			if($_POST['category']!=""){
				$Add_Country=mysql_query("INSERT INTO `animal_minute`.`resource_category` (`Category_ID`, `Category`, `Description`) VALUES (NULL, '$_POST[category]', '$_POST[description]');", $conn);
				echo "<alart>Category <em><strong>'$_POST[category]'</strong></em> was Added</alart>";
			}else{
				echo "<alart>Please enter a Category</alart>";
			}
			echo "</center>";
		}
		?>
        <p>Add Resource Category, its Description, and Click 'Add Recourse Type'</p>
        <div id="contact_form">
        <form action="addresource.php?add=cat" method="post" enctype="multipart/form-data">
        	<label for="category"><strong>Category*:</strong></label>
            <input type="text" name="category" class="required input_field" value=""/>
            <div class="cleaner h10"></div>
        	...
        	<input type="submit" value="Add Recourse Type" id="submit" name="submit" class="submit_btn float_l" /> 
        </form>
        </div>        
        <div class="cleaner"></div>
