...
<title>My Listing | All about Women owned Businesses and More</title>
    ...
    <td><table width="723" border="0" align="right" bgcolor="#FFFFFF">
      <tr>
		...
		if(isset($_GET['view']) && $_GET['view']=="products"){
			echo "<h1>Products</h1>";
			echo "Click Here to <a href=\"mylisting.php?view=animals\">View Businesses</a> that you added";
			echo "<br /><br />";
			...
			$Total_Products = "SELECT Product_ID FROM product WHERE Verified='Yes' AND Added_By=$_SESSION[Member_ID]";
			$totalresults = mysql_query($Total_Products);
			$total=mysql_num_rows($totalresults);
			$total_pgs = ceil($total / $max_results);
			
			$This_Page_SQL = "SELECT Product_ID, Animal, Product, Price, Image FROM product WHERE Verified='Yes' AND Added_By=$_SESSION[Member_ID] ORDER BY Views ASC LIMIT $start_from, $max_results";
		...
		if ($page==1){
			...
			$prev=$page-1;
			echo "<a href='mylisting.php?view=products&page=$prev'>Prev</a>";
		}
		echo " | ";
		if ($page==$total_pgs){
			echo "Next";
			...
			$next=$page+1;
			echo "<a href='mylisting.php?view=products&page=$next'>Next</a>";	
		}
		echo " | ";
		//echo "Prev | Next | ";
		for ($i=1; $i<=$total_pgs; $i++) { 
			if($page==$i){
				...
				echo "<a href='mylisting.php?view=products&page=".$i."'>".$i."</a> "; 
			}
		}
		}
		}else{
			
			echo "<h1>Businesses </h1>";
			echo "Click Here to <a href=\"mylisting.php?view=products\">View Products</a> that you added";
			echo "<br /><br />";
			...
			$Total_Animals = "SELECT Animals_ID FROM animal WHERE Verified='Yes' AND Added_By=$_SESSION[Member_ID]";
			$totalresults = mysql_query($Total_Animals);
			$total=mysql_num_rows($totalresults);
			$total_pgs = ceil($total / $max_results);
			
			$This_Page_SQL = "SELECT Animals_ID, Animal, Breed, Price, Image_1 FROM animal WHERE Verified='Yes' AND Added_By=$_SESSION[Member_ID] ORDER BY Views ASC LIMIT $start_from, $max_results";
			$result = mysql_query($This_Page_SQL, $conn) or die(mysql_error());
			$this_page_total=mysql_num_rows($result);
			$this_page_total_2=mysql_num_rows($result);
			$this_page_total=(($page-1)*$max_results)+$this_page_total;
		...
		if ($page==1){
			...
			$prev=$page-1;
			echo "<a href='mylisting.php?view=animals&page=$prev'>Prev</a>";
		}
		echo " | ";
		if ($page==$total_pgs){
			echo "Next";
			...
			$next=$page+1;
			echo "<a href='mylisting.php?view=animals&page=$next'>Next</a>";	
		}
		echo " | ";
		//echo "Prev | Next | ";
		for ($i=1; $i<=$total_pgs; $i++) { 
			if($page==$i){
				...
				echo "<a href='mylisting.php?view=animals&page=".$i."'>".$i."</a> "; 
			}
		}
		}
	}
